...
def get_xgboost_output(
    model_path: str,
    input_dict: typing.Dict[str, numpy.ndarray],
) -> typing.List[numpy.ndarray]:
    engine = utils.load.load_xgboost(model_path)
    runner = utils.inference.xgboost_inference_runner(engine)

    with utils.common.Profiler("Python xgboost inference", show_latency=True):
...
@utils.save.onnx_saver(
    save_root=os.path.join(utils.project_root(), "models", "hummingbird_xgboost")
)
...
def convert_models(
    ...
    for model_name in tqdm.tqdm(target_models):
        xgb_model_path = os.path.join(
            utils.project_root(), "models", "xgboost", f"{model_name}.xgboost"
        )

        # Convert models
        model_path = convert_function(xgb_model_path, model_name, input_dict)
