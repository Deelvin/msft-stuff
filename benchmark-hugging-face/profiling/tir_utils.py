def get_ir_mod(name):
  if name == "D1":
    ...
    from dense_tir import ModuleDense
    return ModuleDense
  elif name == "BaseAdd":
    from base_add_tir import BaseAdd
    return BaseAdd
  else:
    raise NotImplementedError("Other kernels except D1, D2, Dense, BaseAdd are not supported")
...
def get_rnd_inputs(name):
  if name == "D1":
    ...
    from dense_tir import get_rnd_inputs
    return get_rnd_inputs()
  elif name == "BaseAdd":
    from base_add_tir import get_rnd_inputs
    return get_rnd_inputs()
  else:
    raise NotImplementedError("Random inputs to kernels except D1, D2, Dense, BaseAdd are not supported")
